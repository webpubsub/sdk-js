...
export class WebPubSubError extends Error {
...
export function generatePNSDK(config) {
  ...
  let base = `Webpubsub-JS-${config.sdkFamily}`;
  ...
  if (validationResult) {
    ...
    if (promiseComponent) {
      promiseComponent.reject(
        new WebPubSubError('Validation failed, check status for details', createValidationError(validationResult)),
      );
