...
const endpoint = {
  ...
  handleResponse: async ({ WebPubSubFile, config, cryptography }, res, params) => {
    let { body } = res.response;

    if (WebPubSubFile.supportsEncryptFile && (params.cipherKey ?? config.cipherKey)) {
      body = await cryptography.decrypt(params.cipherKey ?? config.cipherKey, body);
    }

    return WebPubSubFile.create({
      data: body,
