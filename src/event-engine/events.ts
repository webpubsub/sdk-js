...
export const handshakingFailure = createEvent('HANDSHAKING_FAILURE', (error: WebPubSubError) => error);
...
export const receivingFailure = createEvent('RECEIVING_FAILURE', (error: WebPubSubError) => error);

export const reconnectingSuccess = createEvent('RECONNECTING_SUCCESS', (cursor: Cursor, events: any[]) => ({
  cursor,
  events,
}));
export const reconnectingFailure = createEvent('RECONNECTING_FAILURE', (error: WebPubSubError) => error);
