...
function addActions(client        , count        , messageTimetokens               , channel        , completion          ) {
  ...
  const values = [
    Webpubsub.generateUUID(), Webpubsub.generateUUID(), Webpubsub.generateUUID(), Webpubsub.generateUUID(), Webpubsub.generateUUID(),
    Webpubsub.generateUUID(), Webpubsub.generateUUID(), Webpubsub.generateUUID(), Webpubsub.generateUUID(), Webpubsub.generateUUID()
  ];
  let actionsToAdd = [];
  let actionsAdded = 0;
...
describe('message actions endpoints', () => {
  ...
  describe('addMessageAction', () => {
    describe('##validation', () => {
      ...
      it('fails if \'type\' is too long', (done) => {
        ...
        const scope = utils
          ...
          .post(`/v1/message-actions/${subscribeKey}/channel/test-channel/message/1234567890`)
          .reply(200, {});
        const action = { type: Webpubsub.generateUUID(), value: 'test value' };
    ...
    it('add message action', (done) => {
      /** @type MessageAction */
      const messageAction = { type: 'custom', value: Webpubsub.generateUUID() };
      const channel = Webpubsub.generateUUID();

      publishMessages(webpubsub, 1, channel, (timetokens) => {
        webpubsub.addMessageAction(
          ...
          (status, response) => {
            ...
            assert.equal(response.data.uuid, webpubsub.getUUID());
    ...
    it('add message action with encoded channel', (done) => {
      /** @type MessageAction */
      const messageAction = { type: 'custom', value: Webpubsub.generateUUID() };
      const channel = `${Webpubsub.generateUUID()}#1`;
    ...
    it('add message action should trigger event', (done) => {
      ...
      webpubsub.addListener({
        ...
        messageAction: (messageActionEvent) => {
          ...
          assert.equal(messageActionEvent.data.uuid, webpubsub.getUUID());
      ...
      webpubsub.subscribe({ channels: [channel] });
    }).timeout(60000);
  });
  ...
  describe('removeMessageAction', () => {
    ...
    it('remove message action', (done) => {
      ...
      publishMessages(webpubsub, 1, channel, (messageTimetokens) => {
        addActions(webpubsub, 1, messageTimetokens, channel, (actionTimetokens) => {
          webpubsub.getMessageActions({ channel }, (status, response) => {
            assert.equal(status.error, false);
            ...
            webpubsub.removeMessageAction(
              ...
              (removeMessageStatus) => {
                ...
                setTimeout(() => {
                  webpubsub.getMessageActions({ channel }, (getMessagesStatus, getMessagesResponse) => {
  ...
  describe('getMessageAction', () => {
    describe('##validation', () => {
      it('fails if \'channel\' is missing', (done) => {
        ...
        webpubsub.getMessageActions({}).catch((err) => {
          assert.equal(scope.isDone(), false);
          assert.equal(err.status.message, 'Missing message channel');
    ...
    it('fetch message actions', (done) => {
      ...
      publishMessages(webpubsub, 2, channel, (messageTimetokens) => {
        addActions(webpubsub, 3, messageTimetokens, channel, (actionTimetokens) => {
    ...
    it('fetch next message actions page', (done) => {
      ...
      publishMessages(webpubsub, 2, channel, (messageTimetokens) => {
        addActions(webpubsub, 5, messageTimetokens, channel, (actionTimetokens) => {
          const lastPublishedActionTimetoken = actionTimetokens[actionTimetokens.length - 1];
          ...
          webpubsub.getMessageActions({ channel, limit: halfSize }, (status, response) => {
            assert.equal(status.error, false);
