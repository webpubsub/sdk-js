...
describe('access endpoints', () => {
  ...
  beforeEach(() => {
    ...
    webpubsub._config.getVersion = () => 'suchJavascript';
  });
  ...
  describe('#audit', () => {
    it('issues the correct RESTful request for channels', (done) => {
      const scope = utils
        ...
        .query({
          ...
          channel: 'ch1',
          uuid: 'myUUID',
          pnsdk: 'Webpubsub-JS-Nodejs/suchJavascript',
      ...
      webpubsub.audit({ channel: 'ch1' }, (status, response) => {
    ...
    it('issues the correct RESTful request for channel groups', (done) => {
      ...
      webpubsub.audit({ channelGroup: 'cg1' }, (status, response) => {
        assert.equal(status.error, false);
        assert.deepEqual(response, {
          level: 'channel-group+auth',
          subscribe_key: 'mySubscribeKey',
    ...
    it('issues the correct RESTful request for keys', (done) => {
      ...
      webpubsub.audit({ authKeys: ['key1', 'key2'] }, (status, response) => {
        assert.equal(status.error, false);
        assert.deepEqual(response, {
          level: 'channel-group+auth',
          subscribe_key: 'mySubscribeKey',
