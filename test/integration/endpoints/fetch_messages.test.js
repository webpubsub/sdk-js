...
describe('fetch messages endpoints', () => {
  ...
  it('supports payload', (done) => {
    ...
    webpubsub.fetchMessages({ channels: ['ch1', 'ch2'], count: 10 }, (status, response) => {
  ...
  it('supports metadata', (done) => {
    ...
    publishMessagesToChannel(webpubsub, expectedMessagesCount, channel, (messages) => {
      webpubsub.fetchMessages({ channels: [channel], count: 25, includeMeta: true }, (status, response) => {
  ...
  it('throws when requested actions for multiple channels', () => {
    ...
    try {
      webpubsub.fetchMessages({ channels: ['channelA', 'channelB'], includeMessageActions: true }, () => {});
    } catch (error) {
      assert.equal(
        error.message,
  ...
  it("supports actions (stored as 'data' field)", (done) => {
    ...
    publishMessagesToChannel(webpubsub, expectedMessagesCount, channel, (messages) => {
      ...
      addActionsInChannel(webpubsub, expectedActionsCount, messageTimetokens, channel, (actions) => {
        setTimeout(() => {
          webpubsub.fetchMessages({ channels: [channel], includeMessageActions: true }, (status, response) => {
            assert.equal(status.error, false);
  ...
  it('should add fetch messages API telemetry information', (done) => {
    ...
    utils
      .runAPIWithResponseDelays(
        ...
        delays,
        (completion) => {
          webpubsub.fetchMessages({ channels: ['ch1', 'ch2'], count: 10 }, () => {
            completion();
          });
        }
      )
  ...
  it('should return "more" field when server sends it', (done) => {
    ...
    const scope = utils
      ...
      webpubsub.fetchMessages({ channels: ['ch1'], includeMessageActions: true}, (status, response) => {
      assert.equal(status.error, false);
  ...
  it('should request 100 messages when count not provided with single channel', (done) => {
    ...
    const scope = utils
      ...
      webpubsub.fetchMessages({ channels: ['ch1'] }, (status, response) => {
      assert.equal(status.error, false);
      done();
      });
  });
  ...
  it('should request 25 messages when count not provided with multiple channels', (done) => {
    ...
    const scope = utils
      ...
      webpubsub.fetchMessages({ channels: ['ch1', 'ch2'] }, (status, response) => {
      assert.equal(status.error, false);
      done();
      });
  });
  ...
  it('should request provided number of messages when count is specified for history-with-actions', (done) => {
    ...
    const scope = utils
      ...
      webpubsub.fetchMessages({ channels: ['ch1'], includeMessageActions: true, count: 10}, (status, response) => {
      assert.equal(status.error, false);
      done();
      });
  });
  ...
  it('should request provided number of messages when count is specified for batch history with single channel', (done) => {
    ...
    const scope = utils
      ...
      webpubsub.fetchMessages({ channels: ['ch1'], count: 10}, (status, response) => {
      assert.equal(status.error, false);
      done();
      });
  });
