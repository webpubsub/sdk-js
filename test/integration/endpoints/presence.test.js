...
describe('presence endpoints', () => {
  ...
  describe('#whereNow', () => {
    it('returns the requested data for user UUID', (done) => {
      ...
      webpubsub.whereNow({}, (status, response) => {
    ...
    it('returns the requested data for user encoded UUID', (done) => {
      ...
      webpubsubClient.whereNow({}, (status, response) => {
        assert.equal(status.error, false);
        assert.deepEqual(response.channels, ['a', 'b']);
        assert.equal(scope.isDone(), true);
        done();
    ...
    it('returns the requested data for somebody elses UUID', (done) => {
      ...
      webpubsub.whereNow({ uuid: 'otherUUID' }, (status, response) => {
        assert.equal(status.error, false);
        assert.deepEqual(response.channels, ['a', 'b']);
        assert.equal(scope.isDone(), true);
        done();
    ...
    it('returns empty response object when serverResponse has no payload', (done) => {
      ...
      webpubsub.whereNow({ uuid: '' }, (status, response) => {
        assert.equal(status.error, false);
  ...
  describe('#getState', () => {
    it('returns the requested data for user UUID', (done) => {
      ...
      webpubsub.getState({ channels: ['testChannel'] }, (status, response) => {
        assert.equal(status.error, false);
        assert.deepEqual(response.channels, {
    ...
    it('returns the requested for multiple channels', (done) => {
      ...
      webpubsub.getState({ channels: ['ch1', 'ch2'] }, (status, response) => {
        assert.equal(status.error, false);
        assert.deepEqual(response.channels, {
          ch1: { age: 20, status: 'online' },
          ch2: { age: 100, status: 'offline' },
  ...
  describe('#hereNow', () => {
    it('returns response for a single channel', (done) => {
      ...
      webpubsub.hereNow({ channels: ['game1'] }, (status, response) => {
        assert.equal(status.error, false);
        assert.deepEqual(response.channels, {
          game1: {
            name: 'game1',
    ...
    it('returns response for multiple channels', (done) => {
      ...
      webpubsub.hereNow({ channels: ['ch1', 'ch2'] }, (status, response) => {
        assert.equal(status.error, false);
        assert.deepEqual(response.channels, {
          game1: {
            name: 'game1',
    ...
    it('returns response for channel group', (done) => {
      ...
      webpubsub.hereNow({ channelGroups: ['cg1'] }, (status, response) => {
        assert.equal(status.error, false);
        assert.deepEqual(response.channels, {
          ch1: {
            name: 'ch1',
    ...
    it('returns response for global here-now', (done) => {
      ...
      webpubsub.hereNow({}, (status, response) => {
        assert.equal(status.error, false);
        assert.deepEqual(response.channels, {
          bot_object: {
            name: 'bot_object',
    ...
    it('returns response for global here-now with uuids disabled', (done) => {
      ...
      webpubsub.hereNow({ includeUUIDs: false }, (status, response) => {
        assert.equal(status.error, false);
        assert.deepEqual(response.channels, {
          bot_object: {
            name: 'bot_object',
    ...
    it('recovers from false 200 via status', (done) => {
      ...
      webpubsub.hereNow({ includeUUIDs: false }, (status) => {
    ...
    it('reports proper error message from 402 status for GlobalHereNow', (done) => {
      const scope = utils
        ...
        .reply(
          200,
          '{"status": 402, "error": 1, "message": "This feature is not turned on for this account. Contact support@webpubsub.com to activate this feature.", "service": "Presence"}'
        );
      ...
      webpubsub.hereNow({channles: []}, (status) => {
        assert.equal(status.error, true);
    ...
    it('passes arbitrary query parameters', (done) => {
      ...
      webpubsub.hereNow({ channels: ['game1'], queryParameters: { test: 'param' } }, (status, response) => {
        assert.equal(status.error, false);
        assert.deepEqual(response.channels, {
          game1: {
            name: 'game1',
